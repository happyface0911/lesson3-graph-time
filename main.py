...
heatmap_src["요일"] = heatmap_src["날짜"].dt.dayofweek.map(WEEKDAY_MAP)

heatmap_pivot = (
    heatmap_src.groupby(["월", "요일"])["일관객"]
    .sum()
    .reset_index()
...
heatmap_pivot.index = [f"{m}월" for m in heatmap_pivot.index]

fig5 = px.imshow(
    heatmap_pivot,
    color_continuous_scale="Reds",
    aspect="auto",
    labels=dict(x="요일", y="월", color="합계 관객수"),
    title="월 × 요일별 일관객 합계",
)
fig5.update_traces(
    hovertemplate="월: %{y}<br>요일: %{x}<br>합계 관객수: %{z:,}명<extra></extra>"
)
